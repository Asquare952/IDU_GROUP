...
import { MapPin, ShieldAlert, X, AlertTriangle, Lock, ChevronLeft, ChevronRight } from "lucide-react";
...
const page = () => {
 ...
 const [currentSlide, setCurrentSlide] = useState(0);
  const nextSlide = () => {
    setCurrentSlide((prev) => (prev === ActiveProperty.images.length - 1 ? 0 : prev + 1));
  };

  const prevSlide = () => {
    setCurrentSlide((prev) => (prev === 0 ? ActiveProperty.images.length - 1 : prev - 1));
  };
  return (
    <DashboardLayout>
    ...
    <ChevronRight size={22} strokeWidth={2.5} />
            ...
            {ActiveProperty.images.map((_, index) => (
              <div
                ...
                className={`transition-all duration-300 cursor-pointer rounded-full h-1.5 ${
                  index === currentSlide ? "w-4 bg-white shadow-md" : "w-1.5 bg-white/40"
                }`}
              />
            ))}
          </div>
        ...
        <div className="flex flex-col gap-5 p-6 md:p-10 lg:w-[55%]">
          <div className="flex flex-wrap items-center gap-2">
            <div className="bg-[#43A047] text-white py-1.5 px-3 rounded-xl flex items-center gap-2">
              <Lock size={14} />
            ...
            <div className="bg-red-50 text-red-600 py-1.5 px-4 rounded-xl border border-red-100">
              <span className="text-xs font-bold uppercase">{ActiveProperty.hoursRemaining} remaining</span>
            </div>
          </div>

          <div className="space-y-1">
            <h2 className="text-2xl md:text-4xl font-bold text-[#162B4C] leading-tight">{ActiveProperty.title}</h2>
          ...
          <h3 className="text-[#43A047] text-4xl md:text-5xl font-extrabold">
            ...
            <span className="text-gray-400 text-lg md:text-2xl font-normal ml-2">/year</span>
          </h3>

          {/* Progress Bar */}
          <div className="mt-2">
            <div className="flex justify-between items-center mb-2">
              <span className="text-gray-400 text-xs font-bold uppercase tracking-widest">Lock Progress</span>
              <span className="text-[#1B401C] text-sm font-bold">{ActiveProperty.lockProgress}%</span>
            </div>
